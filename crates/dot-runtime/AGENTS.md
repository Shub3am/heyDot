...
- The server listens on `127.0.0.1` only, on a fresh free port for every launch. llama-server binds before it loads the model and answers 503 while loading; losing a port race makes it exit 1, which counts as a crash, and the restart picks another port.
